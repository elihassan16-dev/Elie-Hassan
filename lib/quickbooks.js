...
let _qbMem = null; // warm-instance mirror {map, at}
async function qbCacheMap() {
  if (_qbMem && Date.now() - _qbMem.at < 15000) return _qbMem.map;
  try {
    const { data } = await admin().from("app_settings").select("data").eq("id", "qb_cache").maybeSingle();
    _qbMem = { map: (data && data.data && data.data.map) || {}, at: Date.now() };
  } catch { _qbMem = _qbMem || { map: {}, at: Date.now() }; }
  return _qbMem.map;
}
export async function qbCacheSet(key, data) {
  try {
    const map = await qbCacheMap();
    map[key] = { at: Date.now(), data };
    _qbMem = { map, at: Date.now() };
    await admin().from("app_settings").upsert({ id: "qb_cache", data: { id: "qb_cache", map } });
