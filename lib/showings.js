...
export async function fetchShowings() {
  ...
  // Persist every showing we see, then return the UNION of everything ever seen
  // with the live feed — so a showing that has aged out of the rolling feed (and
  // any lead saved against it) never disappears. Refreshing only adds/updates;
  // it never drops previously-seen showings.
  try {
    const db = admin();
    if (events.length) {
      const rows = events.map((e) => ({ id: showingKey(e), data: e, updated_at: new Date().toISOString() }));
      await db.from("showings").upsert(rows, { onConflict: "id" });
    }
    const { data: stored } = await db.from("showings").select("data");
    if (stored && stored.length) {
      const map = new Map();
      for (const row of stored) { if (row.data) map.set(showingKey(row.data), row.data); }
      for (const e of events) map.set(showingKey(e), e); // freshest details win
      return { configured: true, showings: [...map.values()] };
    }
  } catch (err) {
    // If the store isn't set up yet (or a write fails), fall back to the live
    // feed so showings keep working — just without long-term persistence.
    console.error("[showings] persist failed:", err.message);
  }
  return { configured: true, showings: events };
}
