...
const DETAIL_VARIANTS = [
  (base, id) => `${base}/${id}`,
  (base, id) => `${base}/${id}?include=hashtags`,
  (base, id) => `${base}/${id}/hashtags`,
  (base, id) => `/v2/public/contact/${id}`,
  (base, id) => `/v2/public/contacts/${id}/detail`,
];
...
export async function getContactTags(id) {
  ...
  for (const i of order) {
    try {
      ...
      if (out.tags.length || out.det) { workingDetail = i; return { ok: true, ...out }; }
    } catch { /* try next */ }
  }
  return { ok: false };
}
...
export async function probeDetail(id) {
  ...
  for (const make of DETAIL_VARIANTS) {
    ...
    try {
      ...
      out.push({ path: path.replace(String(id), "{id}"), status: r.status, ok: r.ok, tags: d ? d.tags.length : undefined, fields: d && d.det ? Object.keys(d.det).slice(0, 40) : undefined, msg: r.ok ? undefined : r.body });
