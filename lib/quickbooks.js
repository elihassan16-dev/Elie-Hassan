...
export async function qbApi(path) {
  ...
  await bumpUsage(); // Intuit meters successful data-out calls — count what they count
  return r.json();
}

// ── Monthly usage gauge ──────────────────────────────────────────────────────
// Intuit meters every successful read under the 500K/month CorePlus cap. We
// count our own calls (row qb_cache_usage) so the app can warn at 80% and
// stretch cache lifetimes instead of ever hitting the wall blind again. The
// gauge is approximate (concurrent lambdas can race an increment) — it's an
// early-warning meter, not an invoice.
export const QB_LIMIT = 500000;
let _use = { month: "", n: 0, at: 0 };
export async function qbUsage() {
  const month = new Date().toISOString().slice(0, 7);
  if (_use.at && Date.now() - _use.at < 30000 && _use.month === month) return _use.n;
  try {
    const hit = await qbCacheGet("usage");
    const n = hit && hit.data && hit.data.month === month ? Number(hit.data.n) || 0 : 0;
    _use = { month, n, at: Date.now() };
    return n;
  } catch { return _use.month === month ? _use.n : 0; }
}
async function bumpUsage() {
  try {
    const month = new Date().toISOString().slice(0, 7);
    const n = (await qbUsage()) + 1;
    _use = { month, n, at: Date.now() };
    await qbCacheSet("usage", { month, n });
  } catch { /* gauge is best-effort */ }
}
...
export async function qbCacheGet(key) {
  const m = _qbMem[key];
...
// "Daily" means every MORNING, not a rolling 24 hours (Elie 9/9: a 7:35 PM
// pull was still "fresh" at 8:35 the next morning). A day starts at 5 AM
// Eastern; a copy pulled before that boundary is stale the first time anyone
// looks after it. daysBack=1 gives the "every other day" tier its boundary.
export function etDayStart(daysBack = 0, hour = 5) {
  const now = Date.now();
  ...
  if (start > now) start -= 86400000;            // before 5 AM: today's window began yesterday
  return start - daysBack * 86400000;
}
// How old a stored copy may be for a request: 0 = force a live pull.
export function qbMaxAge(tier, fresh) {
  if (fresh) return 0;
  if (tier === "sold") return 30 * 86400000;
  return Date.now() - etDayStart(tier === "slow" ? 1 : 0);
}
...
// Past 80% of the monthly quota, cached copies are treated as fresh 6x longer
// so the remaining budget stretches to the 1st (manual ↻ still bypasses via
// maxAgeMs 0).
export async function qbCached(key, maxAgeMs, fetcher) {
  const hit = await qbCacheGet(key);
  if (hit && maxAgeMs > 0) {
    const used = await qbUsage().catch(() => 0);
    const stretch = used > QB_LIMIT * 0.8 ? 6 : 1;
    if (Date.now() - hit.at <= maxAgeMs * stretch) return { data: hit.data, cachedAt: hit.at };
  }
  try {
...
// A QuickBooks "project" is a sub-customer, and one property's costs often
// split between the parent customer and its project twin ("610 Bayview Ave"
// vs "610 Bayview Ave:610 Bayview Ave" — running bills tagged to one, the
// closing COGS re-book to the other). Report filters scope to EXACTLY the
// ids given, so per-property pulls ask for the whole family: the linked id,
// all its descendants, and a parent that shares the same leaf name (a
// same-named parent is the same property, not a grouping).
export async function qbCustomerFamily(customerId) {
  const id = String(customerId);
  try {
    // Include INACTIVE customers: a sold property's project twin is often
    // marked completed/inactive in QBO — exactly the one whose entries the
    // family must still cover. Own cache key, 6h, stale-tolerated.
    const hit = await qbCacheGet("projects_all");
    let list = hit && Date.now() - hit.at < 6 * 3600000 && hit.data?.QueryResponse ? hit.data.QueryResponse.Customer || [] : null;
    if (!list) {
      try {
        const q = "select Id, DisplayName, FullyQualifiedName, Job, ParentRef from Customer where Active in (true, false) maxresults 1000";
        const data = await qbApi(`/query?query=${encodeURIComponent(q)}`);
        await qbCacheSet("projects_all", data);
        list = data.QueryResponse?.Customer || [];
      } catch (e) {
        if (hit && hit.data?.QueryResponse) list = hit.data.QueryResponse.Customer || [];
        else throw e;
      }
    }
    const rows = list.map((c) => ({ id: String(c.Id), name: c.FullyQualifiedName || c.DisplayName || "", parentId: c.ParentRef ? String(c.ParentRef.value) : null }));
    const ids = new Set([id]);
    let grew = true;
    while (grew) {
      grew = false;
      rows.forEach((r) => { if (r.parentId && ids.has(r.parentId) && !ids.has(r.id)) { ids.add(r.id); grew = true; } });
    }
    const leaf = (s) => String(s || "").split(":").pop().trim().toLowerCase().replace(/[^a-z0-9]/g, "");
    const me = rows.find((r) => r.id === id);
    if (me && me.parentId) {
      const par = rows.find((r) => r.id === me.parentId);
      if (par && leaf(par.name) === leaf(me.name)) ids.add(par.id);
    }
    return [...ids];
  } catch { return [id]; }
}
...
// Team-only gate. A contractor's portal login is a real Supabase user, so
// requireAppUser alone would let it call these routes directly. Financials,
// leads and texting are never for contractors (Elie's standing rule) — a
// contractor account, or any account whose role can't be confirmed, is
// treated as signed-out here.
export async function requireTeamUser(req) {
