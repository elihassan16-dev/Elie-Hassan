...
export async function syncBoldtrail() {
  ...
  const fresh = [];
  for (const l of newOnes.slice(0, 25)) {
    const info = await getContactTags(l.id);
    if (!info.ok) continue; // lookup failed — leave unprocessed so next pass retries
    if (!info.tagsConfirmed) continue; // hashtag source not located yet — do not classify
    ...
    if (w.error) { if (dbErrs.length < 4) dbErrs.push("upsert: " + w.error.message); continue; }
    if (!merged.hidden) fresh.push(merged);
  }
  // Only ping about small batches (a real trickle) — the first backlog import
  // stays quiet. And only for properties that are actively selling: buyers
  // tagged to sold/old deals are noise Elie asked not to hear about.
  let alertable = [];
  if (fresh.length && fresh.length <= 12) {
    const norm = (x) => String(x || "").toLowerCase().replace(/[^a-z0-9]/g, "");
    const { data: props } = await db.from("properties").select("address").in("status", ["On Market", "In Closing"]);
    const actAddrs = (props || []).map((x) => norm(x.address)).filter((x) => x.length >= 5);
    const tagAddr = (l) => norm(String((l.tags || []).find((t) => /^pb./i.test(String(t).trim())) || "").trim().replace(/^pb/i, ""));
    alertable = fresh.filter((l) => { const a = tagAddr(l); return a.length >= 5 && actAddrs.some((b) => a.startsWith(b) || b.startsWith(a)); });
  }
  if (alertable.length) {
    const { notifyFanout } = await import("./notify.js");
    for (const l of alertable) {
