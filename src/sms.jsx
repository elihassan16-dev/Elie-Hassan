...
export function SmsFeedSheet({ onClose, isMobile }) {
  ...
  const [filter, setFilter] = useState("all"); // all | agents | buyers | unread
  ...
  return createPortal(
    <>
      <div onClick={onClose} style={{ position: "fixed", inset: 0, zIndex: 450, background: "rgba(0,0,0,0.35)", display: "flex", alignItems: isMobile ? "flex-end" : "center", justifyContent: "center", backdropFilter: "blur(4px)" }}>
        <div onClick={(e) => e.stopPropagation()} style={{ background: "rgba(255,255,255,0.97)", width: isMobile ? "100%" : "min(560px,94vw)", height: isMobile ? "calc(100% - 84px)" : "min(760px,86vh)", borderRadius: isMobile ? "24px 24px 0 0" : 22, boxShadow: "0 -8px 40px rgba(0,0,0,0.2)", display: "flex", flexDirection: "column", overflow: "hidden" }}>
          <div style={{ padding: isMobile ? "10px 16px 0" : "16px 16px 0", flexShrink: 0 }}>
            {isMobile && <div style={{ width: 38, height: 5, borderRadius: 3, background: "#C7C7CC", margin: "0 auto 10px" }} />}
      ...
      {open && <SmsThreadPopup phone={open.phone} name={open.name} sub={open.sub} onClose={() => setOpen(null)} />}
    </>,
    document.body
  );
}
