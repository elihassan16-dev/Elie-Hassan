...
const CANDIDATE_PATHS = [
  "/v2/contacts?perpage=100&sort=-id",
  "/v2/contacts?limit=100",
  "/v2/public/contacts?limit=100",
  "/contacts?limit=100",
];
let workingPath = null; // memoized per warm instance

async function tryFetch(path) {
  const r = await fetch(`${BASE}${path}`, { headers: { Authorization: `Bearer ${btToken()}`, Accept: "application/json", "Content-Type": "application/json" } });
  const j = await r.json().catch(() => null);
  const arr = j && (Array.isArray(j.data) ? j.data : Array.isArray(j.contacts) ? j.contacts : Array.isArray(j) ? j : null);
  return { ok: r.ok && !!arr, status: r.status, arr, sample: arr && arr[0] ? Object.keys(arr[0]).slice(0, 40) : [] };
}

export async function fetchContacts() {
  if (!btToken()) return { configured: false };
  const tried = [];
  const paths = workingPath ? [workingPath, ...CANDIDATE_PATHS.filter((p) => p !== workingPath)] : CANDIDATE_PATHS;
  for (const p of paths) {
    try {
      const r = await tryFetch(p);
      tried.push({ path: p.split("?")[0], status: r.status, ok: r.ok });
      if (r.ok) { workingPath = p; return { configured: true, ok: true, path: p.split("?")[0], contacts: r.arr, sample: r.sample, tried }; }
    } catch (e) { tried.push({ path: p.split("?")[0], error: e.message }); }
  }
  return { configured: true, ok: false, tried };
}
