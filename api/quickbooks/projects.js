...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Projects barely change — a 6-hour shared cache serves every mapping popup.
    const { data, cachedAt, stale } = await qbCached("projects", 6 * 3600000, () => qbApi(`/query?query=${encodeURIComponent(q)}`));
