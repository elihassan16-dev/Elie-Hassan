...
- Phone chrome (Elie approved 9/22/26): top bar = detached floating pill (`gs-topwrap` + `.gs-topbar.gs-pill`, no backdrop-filter — overlays are its DOM children); avatar → `active==="settings"` = `SettingsPage` (profile, NotificationToggle, ProfileMenu `asPage` team rows, Company rows → `SettingsModal initialSection`); no gear on phones. iOS 26/27 (`html.gs-ios26`, feature-detected in App.jsx — the UA OS number is frozen at 18): system frosts ~35pt under the status bar, so `gs-topwrap` gets +24px; `gs-status-edge` strips pin a solid colour under the clock (sign-in gold, app T.bg).
- 🛋 Showroom (`src/showroom.jsx`, NAV key "showroom"): every spec-sheet product in one place (same app_settings row "spec_picks"), departments = SPEC_CATS, one row per product w/ 🔗 store link, auto sub-chips (subOf), `ShowroomPicker` + `RoomsSheet` used by the spec sheet's ＋ Add (multi-select → rooms once). Pictures mirrored to our storage via `api/spec/img` (`mirrorPhoto`/`isOurPhoto` in contractors/spec.jsx); "Recover missing pictures" re-copies old ones.
