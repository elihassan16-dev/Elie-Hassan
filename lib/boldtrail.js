...
// House number + street NAME is what identifies a property — the street-type
// word (and the city that follows it in pb tags) is dropped before matching.
...
export async function storedLeadStats() {
  try {
    ...
    const pbOf = (r) => { const t = ((r.data || {}).tags || []).map(String).find((x) => /^pb./i.test(x.trim())); return t ? core(norm(t.trim().replace(/^pb/i, ""))) : ""; };
    const { data: props } = await db.from("properties").select("address").in("status", ["On Market", "In Closing"]);
    const addrs = (props || []).map((x) => core(norm(x.address))).filter((x) => x.length >= 5);
    ...
    // Near-misses: tag and address agree on the first characters but stop
    // prefix-matching further in — shows exactly where the spellings diverge.
    const near = {};
    rows.forEach((r) => {
      const a = pbOf(r);
      if (!a) return;
      addrs.forEach((b) => {
        if (a.slice(0, 4) === b.slice(0, 4) && !(a.startsWith(b) || b.startsWith(a))) {
          const k = `tag:${a.slice(0, 16)} vs addr:${b.slice(0, 16)}`;
          near[k] = (near[k] || 0) + 1;
        }
      });
    });
...
export async function syncBoldtrail() {
  ...
  const coreA = (x) => { const m = x.match(STREET_SUFFIX); return m ? m[1] : x; };
  const { data: actProps } = await db.from("properties").select("id,address").in("status", ["On Market", "In Closing"]);
  const actList = (actProps || []).map((x) => ({ id: x.id, core: coreA(normA(x.address)) })).filter((x) => x.core.length >= 5);
  const actAddrs = actList.map((x) => x.core);
  const matchPropId = (tags) => {
    const t = String((tags || []).find((x) => /^pb./i.test(String(x).trim())) || "").trim().replace(/^pb/i, "");
    const a = coreA(normA(t));
    if (a.length < 5) return null;
    const hit = actList.find((x) => a.startsWith(x.core) || x.core.startsWith(a));
    return hit ? hit.id : null;
  };
  // Every property regardless of status — used to tell "buyer for a sold deal"
  // (stay silent) apart from "buyer for an address the app doesn't know at all"
  // (alert Elie so the lead isn't silently lost).
  const { data: allProps } = await db.from("properties").select("address");
  const allAddrs = (allProps || []).map((x) => coreA(normA(x.address))).filter((x) => x.length >= 5);
  const tagCore = (tags) => coreA(normA(String((tags || []).find((x) => /^pb./i.test(String(x).trim())) || "").trim().replace(/^pb/i, "")));
  ...
  for (let i = 0; i < batch.length; i += CONC) {
    ...
    await Promise.all(infos.map(async ({ l, info }) => {
      ...
      else if (hasPbTag(tags)) { const c = tagCore(tags); if (c.length >= 5 && !allAddrs.some((b) => c.startsWith(b) || b.startsWith(c))) unplaced.push(merged); }
    }));
  }
  ...
  if (fresh.length && fresh.length <= 12) {
    ...
    for (const l of fresh) {
      ...
      await notifyFanout(db, null, {
        ...
        url: (() => { const pid = matchPropId(l.tags); return pid != null ? `/?goto=showings:${pid}` : "/?goto=showings"; })(),
      }).catch(() => {});
    }
  }
  // A buyer whose tagged address matches NO property in the app (any status)
  // would otherwise vanish silently — ping the admins once per address so the
  // property can be added or its spelling fixed. Notification tag dedups.
  if (unplaced.length && unplaced.length <= 12) {
    const { notifyFanout } = await import("./notify.js");
    const seenCores = new Set();
    for (const l of unplaced) {
      const c = tagCore(l.tags);
      if (seenCores.has(c)) continue;
      seenCores.add(c);
      const raw = String((l.tags || []).find((t) => /^pb./i.test(String(t).trim())) || "").replace(/^pb/i, "");
      const nice = raw.replace(/([a-z])([A-Z0-9])/g, "$1 $2");
      await notifyFanout(db, null, {
        toAdmins: true,
        title: "🧭 Buyer lead has no property to land on",
        body: `${l.name || "Someone"} inquired about ${nice || "an address"} — that address isn't in the app. Add the property (or fix its spelling) and the lead attaches automatically.`,
        tag: `btunplaced-${c}`.slice(0, 64),
        url: "/?goto=showings",
      }).catch(() => {});
    }
  }
