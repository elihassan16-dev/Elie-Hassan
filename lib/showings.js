...
export async function checkNewShowings(feed = null) {
  try {
    ...
    const keyOf = (x) => x.uid || `${x.start}|${x.agent || x.summary || ""}`;
    const first = !Array.isArray(st.seen);
    const seen = new Set(st.seen || []);
    const fresh = data.showings.filter((x) => !seen.has(keyOf(x)));
    const merged = [...new Set([...(st.seen || []), ...data.showings.map(keyOf)])].slice(-2000);
    await db.from("app_settings").upsert({ id: "showing_watch", data: { seen: merged, lastCheck: new Date(now).toISOString() }, updated_at: new Date().toISOString() });
    ...
    for (const x of upcoming.slice(0, 6)) {
      const when = x.start ? new Date(x.start).toLocaleString("en-US", { timeZone: "America/New_York", weekday: "short", month: "short", day: "numeric", hour: "numeric", minute: "2-digit" }) : "";
      await notifyFanout(db, null, {
        ...
        tag: `shownew-${keyOf(x)}`.slice(0, 64),
        url: "/?goto=showings",
      }).catch(() => {});
    }
