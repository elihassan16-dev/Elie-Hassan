...
export async function checkNewShowings(feed = null) {
  try {
    ...
    // ⚙️ Feature switches (Settings portal) — admins can turn these alerts off.
    const { data: featR } = await db.from("app_settings").select("data").eq("id", "features").maybeSingle();
    const flagOn = (k) => !(featR && featR.data && featR.data.flags && featR.data.flags[k] === false);
    if (flagOn("fupReminders")) await checkFollowups(db).catch(() => {});
    if (first || !fresh.length) return { seeded: first, fresh: 0 };
    if (!flagOn("showingAlert")) return { fresh: fresh.length, muted: true };
