...
import { sameFirst } from "./jivetel.js";
...
// pushOnly: skip the email and SMS channels entirely — for high-frequency
// pings (call alerts) where an inbox copy is just noise.
// toSelf: target ONLY the sender — the desktop→phone handoff pings yourself.
// toTeam: every Goldstone login (admins AND members) — never contractors.
// recipientsFirst: target by FIRST name, spelling-tolerant ("esti" finds
// "Estie Unsdorfer") — used when a call/text belongs to one person's line.
const esc = (x) => String(x || "").replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");

// ── Claim an alert ONCE, atomically (Elie 9/24/26: some alerts buzzed twice) ─
// The phone system can deliver the same text / ring / missed call several
// times at the same instant (two encodings, one record per call leg). The old
// guards read a log, checked it, then wrote it — parallel deliveries all read
// before any wrote, so each one notified. Here the database decides: the first
// insert of the alert's id wins, every other copy hits the primary key and
// skips. windowMs buckets the key in time (the same text sent again later is a
// new alert); a claim in the previous bucket also counts, so copies that
// straddle a bucket edge still collapse. Fails OPEN — a hiccup here must never
// cost a real alert. Rows (alert_once_*) are server-only and pruned after a day.
const hashKey = (s) => { let h = 5381; for (const ch of String(s)) h = ((h * 33) ^ ch.charCodeAt(0)) >>> 0; return h.toString(36); };
export const alertTag = (prefix, key) => `${prefix}-${hashKey(key)}`.slice(0, 64);
export async function claimAlert(db, key, { windowMs = 0, at = Date.now() } = {}) {
  const h = hashKey(key);
  let id = `alert_once_${h}`;
  try {
    if (windowMs > 0) {
      const b = Math.floor(at / windowMs);
      id = `alert_once_${h}_${b}`;
      const { data: prev } = await db.from("app_settings").select("id").eq("id", `alert_once_${h}_${b - 1}`).maybeSingle();
      if (prev) return false;
    }
    const now = new Date().toISOString();
    const { error } = await db.from("app_settings").insert({ id, data: { id, k: String(key).slice(0, 120), at: now }, updated_at: now });
    if (error) return !(error.code === "23505" || /duplicate key/i.test(error.message || ""));
    if (Math.random() < 0.05) {
      db.from("app_settings").delete().like("id", "alert\\_once\\_%").lt("updated_at", new Date(Date.now() - 86400000).toISOString()).then(() => {}, () => {});
    }
    return true;
  } catch { return true; }
}

export async function notifyFanout(db, senderId, { recipients = [], recipientsFirst = [], toAdmins = false, toTeam = false, toOrg = null, toSelf = false, title, body, url, tag, pushOnly = false } = {}) {
  const { data: users } = await db.from("users").select("id,email,name,role,contractor_org_id,notify_muted,sms_email,notify_channels");
  // Admin-set per-user channels: {push,email,sms} — a missing object or key means ON.
  const chanOn = (u, k) => { const c = u && u.notify_channels; return !c || c[k] !== false; };
  let targets;
  if (toSelf) {
    targets = (users || []).filter((u) => u.id === senderId); // you asked for it — mute doesn't apply
  } else if (recipientsFirst && recipientsFirst.length) {
    targets = (users || []).filter((u) => u.role !== "contractor" && !u.notify_muted && recipientsFirst.some((n) => sameFirst(n, u.name || u.email)));
  } else if (toTeam) {
    targets = (users || []).filter((u) => u.role !== "contractor" && u.id !== senderId && !u.notify_muted);
  } else if (toAdmins) {
  ...
  const ids = targets.filter((u) => chanOn(u, "push")).map((u) => u.id);
  ...
  if (VAPID_PUBLIC && VAPID_PRIVATE && ids.length) {
    try {
  ...
  if (RESEND && FROM && !pushOnly) {
    const emails = [...new Set(targets.filter((u) => chanOn(u, "email")).map((u) => u.email).filter(Boolean))];
      ...
      try {
        ...
        const r = await fetch("https://api.resend.com/emails", {
          ...
          body: JSON.stringify({
            ...
            // Rich version: what happened + where, and a gold button that
            // deep-links straight to the thing (e.g. the exact task, highlighted).
            html: `<div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;font-size:15px;color:#1C1C1E;line-height:1.6;padding:4px 2px"><div style="font-weight:700;font-size:16px">${esc(title || "Goldstone Properties")}</div><div style="margin:6px 0 16px;color:#3A3A3C">${esc(body || "")}</div><a href="${link}" style="display:inline-block;background:#C9A227;color:#ffffff;text-decoration:none;font-weight:700;font-size:14px;padding:11px 20px;border-radius:12px">Open in Goldstone →</a><div style="margin-top:14px;font-size:12px;color:#8E8E93">The button takes you right to it in the app.</div></div>`,
  ...
  if (ZAP && !pushOnly) {
    const nums = [...new Set(targets.filter((u) => chanOn(u, "sms")).map((u) => String(u.sms_email || "").split("@")[0].replace(/\D/g, "")).filter((d) => d.length === 10))];
  ...
  else if (RESEND && FROM && !pushOnly) {
    const cells = [...new Set(targets.filter((u) => chanOn(u, "sms")).map((u) => (u.sms_email || "").trim()).filter((x) => x.includes("@")))];
