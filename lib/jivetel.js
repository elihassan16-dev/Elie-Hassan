...
export async function identifyPhone(phone) {
  ...
  if (!_who.map || now - _who.at > 5 * 60 * 1000) {
    ...
    try {
      ...
      try { if (SERVICE_ROLE) { const db = createClient(SUPABASE_URL, SERVICE_ROLE, { auth: { persistSession: false } }); const { data } = await db.from("properties").select("data").limit(500); const best = new Map(); (data || []).forEach((r) => { const p = r.data || {}; (p.customLeads || []).forEach((l) => { String(l.phone || "").split(/[\/,;]| or /i).forEach((ph) => { const k = _dig(ph); if (!k || !l.name) return; const at = String(l.at || ""); const prev = best.get(k); if (!prev || at > prev.at) best.set(k, { at, who: { name: l.name, role: l.buyer ? "buyer" : "lead", addr: String(p.address || "").split(",")[0] } }); }); }); }); best.forEach((v, k) => map.set(k, v.who)); } } catch { /* optional source */ }
