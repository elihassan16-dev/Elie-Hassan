...
async function checkFollowups(db) {
  ...
  const due = items.filter((x) => !x.done && !x.notified && x.due && x.due <= today);
  ...
  for (const x of due.slice(0, 6)) {
    await notifyFanout(db, null, {
      ...
      body: `${x.phone}${x.addr ? ` · ${x.addr}` : ""}${x.note ? ` · ${x.note}` : ""}`,
      tag: `fup-${x.id}`, url: "/?goto=showings",
    }).catch(() => {});
  }
  const upd = items.map((x) => due.some((d) => d.id === x.id) ? { ...x, notified: true } : x);
  await db.from("app_settings").update({ data: { items: upd }, updated_at: new Date().toISOString() }).eq("id", "followups");
}
