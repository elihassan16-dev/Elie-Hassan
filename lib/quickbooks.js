...
const CLIENT_ID = process.env.QB_CLIENT_ID || "ABZXasjjiSvG3epOMwQQdq3IfQIns0QUAmIIfM8eVKwYtionqi";
...
export async function qbApi(path) {
  ...
  if (!r.ok) throw new Error(`QuickBooks API ${path} failed (${r.status}): ${await r.text()}`);
  return r.json();
}
