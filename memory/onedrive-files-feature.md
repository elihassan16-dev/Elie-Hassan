...
UPDATE 2026-07-01: Admin consent GRANTED by user's IT. First test hit MSAL **block_nested_popups** (popup login fails in PWA/embedded/popup contexts). FIX: switched Microsoft sign-in from popup to **full-page redirect** — `loginRedirect` + `handleRedirectPromise()` processed at app startup in main.jsx; getToken falls back to `acquireTokenRedirect`; signOut clears locally. No Azure change needed (redirect URI already = origin). Deployed (commit 00e10cb). AWAITING user's re-test of Connect Microsoft → should redirect to MS login, return connected; then paste a folder share link → browse/open/upload. If a permission error appears, verify admin consent covered Files.ReadWrite.All (not just Files.Read.All).
