...
const CONTACTS=[{id:1,name:"Mike Torres",role:"Contractor",phone:"404-555-0101",email:"mike@torres.com"},{id:2,name:"Sarah Bloom",role:"Realtor",phone:"404-555-0192",email:"sarah@realty.com"},{id:3,name:"James Whitfield",role:"Lender",phone:"404-555-0233",email:"james@lend.com"},{id:4,name:"Priya Nair",role:"Attorney",phone:"404-555-0344",email:"priya@law.com"},{id:5,name:"Tom Baxter",role:"Inspector",phone:"404-555-0455",email:"tom@inspect.com"}];
...
function mkLead(o){return{id:Date.now()+Math.random(),address:"",city:"",state:"NJ",zip:"",leadStatus:"New Leads",dateAdded:new Date().toLocaleDateString("en-US",{month:"short",day:"numeric",year:"numeric"}),notes:"",
  tasks:[],contacts:[],
...
function NewLeadsPage({sharedProps,setSharedProps}){
  const[leads,setLeadsRaw]=useState(loadSavedLeads);
  function setLeads(updater){
    setLeadsRaw(prev=>{
      const next=typeof updater==="function"?updater(prev):updater;
      try{localStorage.setItem(LS_LEADS_KEY,JSON.stringify(next));}catch(e){}
      return next;
    });
  }
  const[selId,setSelId]=useState(INIT_LEADS[0].id);
  const[search,setSearch]=useState("");
  const[showAdd,setShowAdd]=useState(false);
...
// ─── Tasks Page ───────────────────────────────────────────────────────────────
const TEAM_MEMBERS=["Mike Torres","Sarah Bloom","James Whitfield","Priya Nair","Tom Baxter"];
const CURRENT_USER="Mike Torres"; // placeholder until real auth

function TasksPage({sharedProps,setSharedProps}){
  const[views,setViews]=useState(new Set(["my"]));
  const[filterMember,setFilterMember]=useState(TEAM_MEMBERS[0]);
  ...
  // ── Automation Builder ──────────────────────────────────────────────────────
  const LS_AUTO_KEY="goldstone_automations_v1";
  const[automations,setAutomationsRaw]=useState(()=>{
    try{const r=localStorage.getItem(LS_AUTO_KEY);return r?JSON.parse(r):[];}catch(e){return[];}
  });
  function setAutomations(next){
    setAutomationsRaw(next);
    try{localStorage.setItem(LS_AUTO_KEY,JSON.stringify(next));}catch(e){}
  }
    ...
    <div style={{flex:1,display:"flex",flexDirection:"column",background:T.bg,overflow:"hidden"}}>
      ...
      <div style={{background:T.card,borderBottom:bdr,padding:"18px 28px",flexShrink:0}}>
        ...
        <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
          {[["my","My Tasks"],["assigned","Assigned by Me"],["member","By Team Member"],["unassigned","Unassigned"],["all","All Tasks"],["automations","⚙ Automations"]].map(([k,l])=>{
...
// ─── App Shell ────────────────────────────────────────────────────────────────
const LS_KEY = "goldstone_props_v2";
const LS_LEADS_KEY = "goldstone_leads_v1";
function loadSaved(){
  try{
    const raw=localStorage.getItem(LS_KEY);
    if(raw){
      const saved=JSON.parse(raw);
      // Merge saved data onto INIT_PROPS so new fields from code updates are included
      return INIT_PROPS.map(p=>{
        const s=saved.find(x=>x.id===p.id);
        return s?{...p,...s,financials:{...p.financials,...s.financials},propertyInfo:{...p.propertyInfo,...s.propertyInfo}}:p;
      }).concat(saved.filter(s=>!INIT_PROPS.find(p=>p.id===s.id))); // keep any user-added props
    }
  }catch(e){}
  return INIT_PROPS;
}
function loadSavedLeads(){
  try{
    ...
    if(raw){
      const saved=JSON.parse(raw);
      // saved is the full authoritative list (includes deletions/conversions already applied)
      return saved.map(s=>{
        const base=INIT_LEADS.find(l=>l.id===s.id);
        return base?{...base,...s,financials:{...base.financials,...s.financials},info:{...base.info,...s.info},propertyInfo:{...base.propertyInfo,...s.propertyInfo}}:s;
      });
    }
  }catch(e){}
  return INIT_LEADS;
...
export default function App(){
  ...
  const[sharedProps,setSharedPropsRaw]=useState(loadSaved);
  const[navPropId,setNavPropId]=useState(null);

  function setSharedProps(updater){
    setSharedPropsRaw(prev=>{
      const next=typeof updater==="function"?updater(prev):updater;
  ...
  return(
    <div style={{display:"flex",height:"100vh",width:"100%",fontFamily:"-apple-system,BlinkMacSystemFont,'SF Pro Display','Segoe UI',sans-serif",background:T.bg,overflow:"hidden"}}>
      <aside style={{width:220,background:T.card,borderRight:`1px solid ${T.border}`,display:"flex",flexDirection:"column",flexShrink:0}}>
        ...
        <nav style={{flex:1,padding:"12px 10px"}}>
          {NAV.map(({key,label,icon})=>{const isActive=active===key;return <button key={key} onClick={()=>setActive(key)} style={{display:"flex",alignItems:"center",gap:12,width:"100%",padding:"10px 12px",borderRadius:T.radiusSm,border:"none",background:isActive?T.goldLight:"transparent",color:isActive?T.gold:T.textSub,fontWeight:isActive?600:400,fontSize:14,cursor:"pointer",marginBottom:2,transition:"all 0.15s",textAlign:"left",fontFamily:"inherit"}}><span style={{color:isActive?T.gold:T.textTert}}>{icon}</span>{label}</button>;})}
        </nav>
        <div style={{padding:"14px 16px",borderTop:`1px solid ${T.border}`,display:"flex",alignItems:"center",gap:10}}>
          <div style={{width:34,height:34,borderRadius:"50%",background:`linear-gradient(135deg,${T.gold},${T.goldMid})`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:14,fontWeight:700,color:"#fff",flexShrink:0}}>A</div>
          <div><div style={{fontSize:13,fontWeight:600,color:T.text}}>Admin</div><div style={{fontSize:11,color:T.textSub}}>Owner</div></div>
        </div>
      </aside>
      <main style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
        <div style={{height:54,padding:"0 24px",borderBottom:`1px solid ${T.border}`,background:T.card,display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0}}>
        ...
        <div style={{flex:1,overflow:"hidden",display:"flex",flexDirection:"column"}}>
          {active==="properties"?<PropertiesPage sharedProps={sharedProps} setSharedProps={setSharedProps} initialSelId={navPropId} onNavConsumed={()=>setNavPropId(null)}/>:active==="leads"?<NewLeadsPage sharedProps={sharedProps} setSharedProps={setSharedProps}/>:active==="portfolio"?<PortfolioPage sharedProps={sharedProps} setSharedProps={setSharedProps} onNavigate={navigateToProperty}/>:active==="tasks"?<TasksPage sharedProps={sharedProps} setSharedProps={setSharedProps}/>:<ComingSoon label={NAV.find(n=>n.key===active)?.label}/>}
        </div>
      </main>
    </div>
  );
