...
// Create a new signed-in-able team member (email + password). email_confirm:true so
...
export async function createTeamMember({ name, email, password }) {
  ...
  if (uid) {
    await client.from("users").upsert({ id: uid, email, name: finalName, role: "member" }, { onConflict: "id" });
  }
  return { id: uid, email, name: finalName, role: "member" };
}
