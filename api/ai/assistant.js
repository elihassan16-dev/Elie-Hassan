  ...
  try {
      ...
      tools: [{
        ...
        input_schema: {
          ...
          properties: {
            ...
            offerPrice: { type: "string", description: "The price WE would pay — the user's own offer/underwriting number ('my offer', 'I'd pay', 'we're in at', 'buy at'). Full plain number" },
            holdPeriod: { type: "string", description: "Planned hold / project length in MONTHS if stated ('6 month flip', 'in and out in 5'). Just the number of months" },
    ...
    if (!action && lu) {
      ...
      const lead = {
        ...
        rehabEstimate: money(lu.input?.rehabEstimate), offerPrice: money(lu.input?.offerPrice), holdPeriod: digits(lu.input?.holdPeriod), closingTarget: s(lu.input?.closingTarget),
