...
export async function identifyPhone(phone) {
  ...
  if (!_who.map || now - _who.at > 5 * 60 * 1000) {
    ...
    try {
      ...
      // Agents: prefer the MOST RECENT showing. The same agent shows several
      // of our properties, and last-write-wins used to label them with
      // whichever property iterated last (Rachel got "420 Philadelphia" on a
      // 9 Fourteenth St conversation). Snapshots, hand-attached numbers and
      // the live feed all merge by showing date, newest wins.
      try {
        const bestShow = new Map(); // key -> {at, who}
        const consider = (k, at, who) => { if (!k || !(who.name || who.addr)) return; const prev = bestShow.get(k); if (!prev || String(at || "") > String(prev.at || "")) bestShow.set(k, { at: String(at || ""), who }); };
        propRows.forEach((p) => {
          const addr = String(p.address || "").split(",")[0];
          Object.entries(p.showingSnapshots || {}).forEach(([sk, sn]) => {
            String(sn.phone || "").split(/[\/,;]| or /i).forEach((ph) => consider(_dig(ph), sn.start, { name: sn.agent || "", role: "agent", addr }));
            (((p.showingPhones || {})[sk]) || []).forEach((ph) => consider(_dig(ph), sn.start, { name: sn.agent || "", role: "agent", addr }));
          });
          Object.entries(p.showingPhones || {}).forEach(([sk, list]) => { if ((p.showingSnapshots || {})[sk]) return; (list || []).forEach((ph) => consider(_dig(ph), "", { name: "", role: "agent", addr })); });
        });
        try { const { fetchShowings } = await import("./showings.js"); const d = await fetchShowings(); (d.showings || []).forEach((s) => { String(s.phone || "").split(/[\/,;]| or /i).forEach((ph) => consider(_dig(ph), s.start, { name: s.agent || "", role: "agent", addr: String(s.location || s.summary || "").split(",")[0] })); }); } catch { /* feed down — cache what we have */ }
        bestShow.forEach((v, k) => map.set(k, v.who));
...
export async function checkOutreach(phone, senderName, kind) {
  try {
    ...
    // Keep the id embedded in data — the app finds this row by it (a missing
    // id turns the app's saves into duplicate-key inserts).
    if (row && row.data && !row.data.id) {
      await db.from("app_settings").update({ data: { id: "contact_approvals", items: all }, updated_at: new Date().toISOString() }).eq("id", "contact_approvals").then(() => {}, () => {});
    }
    const ent = all[p] || {};
    ...
    if (!mine || Date.now() - new Date(mine.at || 0).getTime() > 4 * 3600000) {
      ...
      await db.from("app_settings").upsert({ id: "contact_approvals", data: { id: "contact_approvals", items: all }, updated_at: new Date().toISOString() });
